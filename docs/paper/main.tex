...
\begin{abstract}
Deforestation in an isolated island, Sumba, is investigated using a coupled model between human, forest, and climate. To represent the role of climate stress in shaping the ecosystem, forest growth is reduced during El Niño events through a climate-dependent suppression term. In general, the model predicts a remaining time of 3–8 decades before societal collapse under a constant consumption rate. When El Niño forcing is included, collapse occurs earlier by approximately 41–52 years compared to the model without climate interactions.
\end{abstract}
...
The equilibria $(N_2, R_2)$ and $(N_3, R_3)$ exist under the condition $\gamma E < r'$. Additionally, $(N_2, R_2)$ also exists given $E\neq0$. By studying the Jacobian matrix at these equilibrium points, it can be concluded that $(N_2, R_2)$ is locally unstable, while $(N_3, R_3)$ is locally stable.
...
\caption{Upper plot: Solution to Eq \eqref{human_eq} with $N_0=780,000$ persons and initial time $t=2020$; Lower plot: Solution to Eq \eqref{forest_eq} with $R_0=5,023$ km${}^{-1}$. Different solutions are plotted given varying values of $\gamma_0$. Thick lines indicate ensemble mean, while shaded area corresponds to the 95\% interval of the ensemble simulation.}
\label{fig:ts}.
\end{figure}
